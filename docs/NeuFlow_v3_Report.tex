...
cached frame at 7.7$\times$ lower cost, and a calibrated confidence value per
...
\subsection{Cropping: fixed accuracy cost, device-dependent speed}
\begin{center}
\small
\begin{tabular}{lrrrr}
\toprule
...
\midrule
RTX 4060 (laptop) & 33.3\ms & 7.6\ms & \textbf{4.38$\times$} & +0.034\px \\
Tesla V100 & 17.8\ms & 15.1\ms & 1.18$\times$ & +0.034\px \\
\bottomrule
\end{tabular}
\end{center}

Same script, same 40 pairs, same margins. The accuracy cost is identical to three
decimals because it is the same computation; only the speed benefit differs. The
V100 completes a full frame in 17.8\ms\ and is bound by the number of kernel
launches rather than by pixels, so removing 86\% of the area removes almost no
work. The weaker device is compute bound and converts the reduction nearly in
full. This points the right way for deployment: the technique pays off on
constrained hardware and looks pointless on a datacentre GPU, so the figure has
to be quoted per device.
...
v2 is the better choice on every axis: more accurate, and 14\% faster in that mode.
