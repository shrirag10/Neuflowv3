...
\title{NeuFlow v3: Queried Optical Flow with\\ Calibrated Uncertainty for
...
flow, including image registration, sparse tracking and survey mosaicking, need a
few hundred correspondences at locations they select rather than a value at every
pixel, and a platform on a millisecond budget can afford neither the full field
nor its recomputation. We present NeuFlow v3, which retains the NeuFlow v2
pipeline through its coarse flow stage and replaces only the final convex
upsampler with an implicit decoder evaluated at arbitrary continuous coordinates.
The front end remains frozen, verified identical after every run, so each
measured difference is attributable to the new stage alone. We pursue three
objectives. Queryable output makes cost scale with the number of points requested
rather than with image area. Cheap repeatability lets a frame pair be processed
once and every later query answered from cached state in 1.25 milliseconds
against 33.3, a factor of twenty-seven. Calibrated confidence returns a per-query
error scale the baseline cannot express. Declining the least confident fifth of
queries reduces mean error across the remaining eighty percent of the frame to
1.06 pixels, a selection the baseline cannot make at any threshold. The decoder
costs 2.6 percent of mean accuracy from a model thirteen percent smaller. We
further show that the context margin a cropped region requires is set by
inter-frame displacement rather than image size, verified at two motion scales
differing by a factor of three.
\end{abstract}

\begin{IEEEkeywords}
Implicit neural representations, motion estimation, optical flow, real-time
systems, uncertainty quantification
\end{IEEEkeywords}
...
Fig.~\ref{fig:scenarios} shows the three situations this work is measured on.
...
\textbf{Training.} All runs are generated from a single template. Optimisation is
held fixed throughout: seed 1234, batch size 16, learning rate
...
evaluation configuration, 100{,}000 steps, backbone frozen throughout. Crop size
follows the training stage, at $384{\times}512$, $320{\times}512$ and
$320{\times}496$ respectively, because one crop must serve every sub-dataset in a
mixed batch; only the uncertainty run is therefore a single-variable comparison
...
\caption{300 queries at detected corners, decoded in one call: 0.06\% of the
...
Table~\ref{tab:speed} reports latency for each access pattern.
