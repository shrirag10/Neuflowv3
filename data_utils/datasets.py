...
class VKITTI2(FlowDataset):
    ...
    Expected directory layout (after download & extract)::
        ...
        │   ├── clone/frames/forwardFlow/Camera_0/flow_00000.png
        │   └── ...
        ├── Scene02/ ...
        └── Scene20/ ...

    Flow is stored as 16-bit PNGs (same format as the original VKITTI),
    which are read by ``frame_utils.read_vkitti_png_flow``.
    """
        ...
        for scene in scenes:
            for variant in variants:
                ...
                images = sorted(glob(osp.join(img_dir, '*.jpg'))) + \
                         sorted(glob(osp.join(img_dir, '*.png')))
...
def build_train_dataset(stage):
    if stage == 'chairs':
        ...
        # VKITTI2 training stage — intended for implicit-decoder prototyping.
        crop_size = (256, 512)
